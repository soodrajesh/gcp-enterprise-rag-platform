...
## T15 · CI deploy job would fail keyless auth: GitHub immutable OIDC subject claims
Found while testing the sister repo (`gcp-gke-platform-engineering`, see its runbook 09 · L9). New GitHub repositories issue **immutable subject claims** (`repo:<owner>@<id>/<repo>@<id>:environment:prod`), so a Workload Identity binding on the literal name-based subject (`repo:<owner>/<repo>:environment:prod`) never matches: auth succeeds, then `iam.serviceAccounts.getAccessToken` is denied. **Fix (applied to `terraform/modules/github_wif`):** map `attribute.environment` and bind `roles/iam.workloadIdentityUser` to `principalSet://…/attribute.environment/prod`; the repo restriction stays in the provider `attribute_condition`. *Status: the identical change was proven live on the GKE repo; this repo's WIF module was updated the same way but not re-deployed.*
